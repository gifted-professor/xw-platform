...
## 10. M6-2 执行中状态（W1–W6 离线，Plan V2 `READY_FOR_EXECUTION`）

M6-2 走完零 live 前六周（W1 契约/W2 strict freezer/W3 evidence store/W4 真实只读观察/W5 封闭 gate+facade/W6 CLI/CI/docs/release proof），全部 gate 绿。M6-2 只做执行级帧：A/B 位精确原图 + 封闭只读设备观察，禁止任何坐标/action/shell/URL 进出 M6 面。

- **W1 增量契约**：`m6-contracts.mjs`（schemas）未动；`vision-inventory.v1.json` 登记 `packages/kernel/lib/m6-screen-frame.mjs`，`visual-assets.lock` 更新 `hermetic-fixture-provider` sha。
- **W2 唯一 strict frame freezer**：`packages/kernel/lib/m6-screen-frame.mjs`。replay（M6-1 GroundingRuntime）与 live_strict 共用同一 manifest/frameId 派生，`mode` 必填、无默认。live_strict 硬件门：A/B 全字节 SHA-256 相等（无裁切/无遮罩/无感知阈值）、PNG IHDR 维度=display observation、A→B skew ≤4s、B→focusB ≤1s、capturedAt=focus-B 完成时刻、`expiresAt=capturedAt+5s`（截断 .000）、返回时 TTL ≥2s 否则 `M6_FRAME_TTL_EXPIRING`。replay 字节级兼容证明：同 bundle 产出同 frameId `dce2de0f…`。
- **W3 fail-closed evidence store**：`services/control-plane/control-plane/lib/m6-frame-evidence-store.mjs`——CAS 内容寻址（`att-<kind>-<sha256>`）、原子写（temp `wx`+fsync+rename+readback hash）、per-kind 校验（完整 PNG 走 chunk walk、dump 走 tag-stack well-formed、focus 文本、observation JSON）、tombstone/retention/崩溃残留清扫、路径逃逸（junction/symlink）拒绝。**无 debt/stub 模式**：任何证据失败 → 无帧。12 项测试全绿（含真实 Windows junction 逃逸）。
- **W4 真实只读 Xiaowei adapter**：`services/control-plane/apps/xiaowei/read-observation.mjs`（唯一 M6-2 设备接触面）+ 能力 `xiaowei.m6.observe_frame`（effect=none、canaryOnly、封闭空参 schema）+ `explorer-primitive.mjs` 抽出共享只读 helpers（`readScreenPng`/`readDumpXml`/`readWindowFocusText`，唯一实现，行为不变 9/9）。固定顺序 screenshot A→focus A→dump→screenshot B→focus B 全程包在**一次** `runExclusive` 临界区（同 session/lease、30s 预算）；全部 shell 命令来自 `M6_READ_COMMANDS` 封闭注册表（只读 `dumpsys window/activity/display/power/input_method` grep，调用方只能传 name）；`tap/swipe/back/launch_app/input_text/keyevent` 在观察模块静态不可达。字段源矩阵全由真实 fixtures 覆盖：`tests/fixtures/m6-xiaowei/`（1080×2400 真实结构 PNG×2 位级相同 + dumpsys/uiautomator 原始输出文本），观察器自身 parser 从原始文本读字段，无 hand-fill。11 项测试全绿。W6 才给新 services 文件加 fusion allowlist 条目（verify/test-gate 当前已 PASS / KNOWN_FAILURE_MATCH）。
- **W5 唯一 live gate + 封闭 facade**：
  - `services/control-plane/control-plane/lib/m6-live-gate.mjs`（纯模块，不读设备/session/lease/fs）：不可变内容寻址 epoch 链（`epochHash = sha256("xw.m6-live-gate.v1:epoch:"+canonicalJson(payload))`，payload 排除 epochHash/closeoutRef）。`mode` 仅 CLOSED/OBSERVE_ONLY、无激活路径；tail epoch 为 active；任何 missing/unknown/forged/expired/parent-mismatch/release-drift/lock-mismatch → fail-closed 返回 CLOSED + 首个 M6_GATE_* 错误码。allowlist 为 server 解析的 alias 句柄（非 device id/token/coordinate）。
  - `services/control-plane/control-plane/lib/m6-frame-capture.mjs`（facade，434 行）——**M6-2 唯一公开面**，只有四个封闭操作 `preflight/capture/status/closeout`，且只接受 `alias + scenarioLabel(enum ["observe"]) + idempotencyKey`。capture 建真实 server-owned capability session（`control.createSession`，one-device-one-lease）+ 真实 queued job，经新公开 seam `control.m6RunFrameJob` 调私有 `#runJob`；`resultSummary` 会丢弃 observation → 用 `onVerified({execution})` 回调捕获完整 observation；finally 收敛：取消 queued job + `control.releaseSession`（不返回 token）。audit trail（`auditRoot/<attemptId>.json` 原子写）绑定 epochHash + attempt/job/session/lease refs；closeout 写独立 `<attemptId>.closeout.json` seal。`redactM6Output` 幂等，绝不泄漏 token/sessionId/serial。
  - `control-plane.mjs` 新增 `m6RunFrameJob` seam：只接受 `/^xiaowei\.m6\./` 能力 id + `canary===true` 的 job（`M6_JOB_SCOPE_INVALID`/`M6_JOB_CANARY_REQUIRED`），`restore`/`releaseLease` 不触发 return-home（observe_frame 为 lab_only）。
  - `bootstrap.mjs` 新增 `m6Enabled`（默认 off，保 legacy byte-identical）+ `loadLegacyCompatProfile()`（读 `packages/kernel/contracts/runtime-profile.v1.json` 的 `legacy_compat.agenticGroundingEnabled`，已 true）；`router.mjs` 加四个 M6 route + `closedM6Input`（白名单字段之外 → `M6_INPUT_CLOSED` 400）+ 无 facade → 503 `M6_FACADE_UNAVAILABLE`（绝无降级回退）。
  - **发现并修复 W2 gap**：strict frame 契约要求 `mode` 必填无默认，但 `buildCanonicalFrame` 未将其写回 frame 且 schema 未收 `mode`。已将 `mode`(replay|live_strict) 写入 `packages/kernel/lib/m6-screen-frame.mjs` 输出的 frame + `xw.screen-frame.v1.schema.json` required/properties（`additionalProperties:false` 不变），并更新 `screen-frame.valid.json` fixture 与 `vision-inventory.v1.json` 的 sha256（仅一处，源文件本身）。
  - **W5 测试**：`tests/m6-frame-capture.test.mjs`（16 项）+ `tests/control-plane-server.test.mjs` 两个 bootstrap 接线测试（m6Enabled=off 时 M6 命名空间 503；m6Enabled=on 时 facade 安装、无 facade 绝无降级、公开面仅 observe capture）。覆盖：gate CLOSED/expired/parent-mismatch/release-drift/forged-self-hash/forged-closeout、profile missing/false、alias 越界、fixture-provider 结构禁止、同 alias 竞争（M6_LEASE_CONFLICT）、证据失败收敛到 rejected audit + session 释放、`closedM6Input` 拒收 action/coordinate（M6_INPUT_CLOSED）、closedM6 里 tap/swipe/keyevent/bounds/http 静态扫描为 0。**验收达标**：gate/profile 不满足时 transport read count=0；M6 公开面只有 observe capture。gate unit 12 断言单测 + 全 16 项全绿。
- **零 live 重申**：`M6_AGENTIC_LIVE_GATE` 保持 CLOSED、未碰真机、未改 DB schema/enum、未发 release；`xiaowei.m6.observe_frame` 与全部 M6-2 面均为 canary-only 离线。
- **回归基线**：control-plane 836 项（fail 28，与 pristine HEAD 全同——环境既有，非本工作引入；新增 44+21 项零新增失败）；orchestrator 527 项全绿；`fusion:verify` PASS（blobMismatch 0 / missing 0 / extra 0）；`test:gate` KNOWN_FAILURE_MATCH（unexpectedFailures []）。
- **W6 CLI/CI/docs/release profile proof**：
  - `packages/control-client/lib/control-client.mjs` 新增四个封闭 M6 客户端方法（`m6Preflight/m6Capture/m6Status/m6Closeout`）——唯一 M6 客户端面，只转发 alias/scenarioLabel/idempotencyKey，绝无坐标/action token/session token。
  - `packages/cli/xw.mjs` 新增 `xw m6 frame preflight|capture|status|closeout` 薄入口（`m6FrameMain`）：JSON 输出机器可读且 `redact()` 脱敏，默认不循环、不翻 gate。`node packages/cli/xw.mjs m6 frame --help` 可用。
  - `services/control-plane/tests/m6-live-gate.test.mjs`：21 项纯 gate 单元测试（mode 只产 CLOSED/OBSERVE_ONLY、forged/expired/parent/dup/chain-too-long/release-drift/lock-drift/allowlist/closeout-seal 全部 fail-closed 到 CLOSED）。
  - `package.json` 新增 `test:m6-2:offline`（= 控制平面 M6 五个测试文件，94 项全绿）；`.github/workflows/source-fusion.yml` 双平台新增 `M6-2 zero-live offline frame capture` step（无 `continue-on-error`）。
  - **release profile proof**：`xw cutover preflight --release` 新增 `release:m6-2:runtime-profile` + `release:m6-2:agentic-grounding-enabled` 两个检查，直接从打包 release 的字节读取 `packages/kernel/contracts/runtime-profile.v1.json`，验证 `runtimeProfile=legacy_compat` 且 `agenticGroundingEnabled=true`（从不读 live runtime）。已用真实 `cutover package --release-id m6-2-w6-proof`（903 文件）+ preflight 端到端验证 PASS。
  - **fusit allowlist**：`docs/fusion/post-import-allowlist.v1.json` 的 `control-plane.allowedExtra` 增 16 个新 M6 文件（read-observation/m6-frame-capture/m6-frame-evidence-store/m6-live-gate + 4 测试 + 8 fixtures），`allowedBlobModified` 增 5 个改动的 tracked 文件（adapter/capabilities/explorer-primitive/bootstrap/server-test）。`fusion:verify` 当前 PASS。
