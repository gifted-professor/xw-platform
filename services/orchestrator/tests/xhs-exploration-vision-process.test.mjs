...
function fixture(t, scriptBody, { maxBufferBytes = 64 * 1024, spawnImpl = spawn } = {}) {
  ...
  const analyzer = createPinnedExplorationVisionAnalyzer(config, { stagingRoot, spawnImpl });
...
test("pinned analyzer spawns without a shell and analyzes a private exact-byte staging copy", async (t) => {
  ...
  const { analyzer, stagingRoot } = fixture(t, `
    import { createHash } from "node:crypto";
    ...
    writeFileSync(output, JSON.stringify({
      frameHash,
      elements: [{ label: "暂停", bounds: [100, 300, 200, 160], conf: 0.91 }],
    }));
