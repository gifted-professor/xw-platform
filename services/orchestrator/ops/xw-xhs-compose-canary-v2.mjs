#!/usr/bin/env node
/**
 * Four-device XHS compose canary v2 with a persisted run-event timeline.
 *
...
import {
  ...
  finalizeLiveAttemptReport,
  formatLiveEvent,
} from "../scripts/lib/xhs-run-report.mjs";

const XHS_PKG = "com.xingin.xhs";
const SCRIPT_PATH = fileURLToPath(import.meta.url);
let workerEvidenceFailure = null;
let workerEventRequestSeq = 0;
const workerEventAcks = new Map();

function evidenceFailureError(message) {
  const error = new Error(String(message || "parent timeline persistence failed"));
  error.code = "XHS_RUN_EVENT_PERSISTENCE_UNCONFIRMED";
  error.stopClass = "evidence_failure";
  return error;
}

function sendWorkerEvent(draft, { awaitPersisted = false, timeoutMs = 10_000 } = {}) {
  if (typeof process.send !== "function") {
    const error = new Error("worker evidence IPC unavailable");
    error.code = "XHS_RUN_EVENT_IPC_UNAVAILABLE";
    error.stopClass = "evidence_failure";
    throw error;
  }
  const requestId = `worker-event-${process.pid}-${++workerEventRequestSeq}`;
  const message = { type: "xhs-run-event", requestId, draft: { ...draft, source: "worker" } };
  if (!awaitPersisted) {
    process.send(message, (error) => {
      if (error && !workerEvidenceFailure) workerEvidenceFailure = String(error.message || error);
    });
    return null;
  }
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      workerEventAcks.delete(requestId);
      const error = evidenceFailureError(`timeline persistence acknowledgement timed out for ${draft.event}`);
      workerEvidenceFailure ||= error.message;
      reject(error);
    }, timeoutMs);
    workerEventAcks.set(requestId, {
      resolve: (event) => { clearTimeout(timer); resolve(event); },
      reject: (error) => { clearTimeout(timer); reject(error); },
    });
    process.send(message, (error) => {
      if (!error) return;
      const pending = workerEventAcks.get(requestId);
      if (!pending) return;
      workerEventAcks.delete(requestId);
      const failure = evidenceFailureError(error.message || error);
      workerEvidenceFailure ||= failure.message;
      pending.reject(failure);
    });
  });
}

function emitWorkerEvent(draft) {
  return sendWorkerEvent(draft);
}

function persistWorkerEvent(draft) {
  return sendWorkerEvent(draft, { awaitPersisted: true });
}

function handleWorkerParentMessage(message) {
  if (message?.type === "xhs-run-evidence-failure") {
    workerEvidenceFailure = String(message.error || "parent timeline persistence failed");
  }
  if (message?.type !== "xhs-run-event-ack") return;
  const pending = workerEventAcks.get(message.requestId);
  if (!pending) return;
  workerEventAcks.delete(message.requestId);
  if (message.ok) {
    pending.resolve(message.event || null);
    return;
  }
  const error = evidenceFailureError(message.error);
  workerEvidenceFailure ||= error.message;
  pending.reject(error);
}

function assertWorkerEvidenceHealthy() {
  if (!workerEvidenceFailure) return;
  const error = new Error(workerEvidenceFailure);
  error.stopClass = "evidence_failure";
  throw error;
}

export async function runFailureCleanup({ persistEvent, restore }) {
  try {
    await persistEvent("cleanup_started");
  } catch { /* cleanup must proceed even when timeline persistence is unavailable */ }
  try {
    restore();
  } catch {
    return false;
  ...
  try {
    await persistEvent("cleanup_finished");
  } catch { /* raw command records and parent release remain the fallback */ }
  return true;
}

function usage() {
  return `usage:
  node ops/xw-xhs-compose-canary-v2.mjs [--aliases 01,02,03,04] [--keyword 夏季穿搭]
  node ops/xw-xhs-compose-canary-v2.mjs --aliases 01,02,03,04 --run <runId> --actor <actor> [--attempt 2] --execute --canary-authorized
...
const ZERO_EFFECT_COMMANDS = Object.freeze({
  like_note: "ops/xhs-like-one.mjs",
  collect_note: "ops/xhs-collect-one.mjs",
  follow_author: "ops/xhs-follow-one.mjs",
});

export function assertZeroEffectCommand(actionId, command) {
  const requiredScript = ZERO_EFFECT_COMMANDS[actionId];
  if (!requiredScript) return command;
  const dryRunCount = command.filter((token) => token === "--dry-run").length;
  if (command[0] !== requiredScript || dryRunCount !== 1) {
    const error = new Error(`${actionId} is not bound to its exact dry-run command`);
    error.stopClass = "unexpected_effect_risk";
    throw error;
  }
  return command;
}
...
function runLogged(args, { alias, actionId, logDir, timeoutMs = 240_000 } = {}) {
  ...
  emitWorkerEvent({
    event: "command_started", status: "running", alias, actionId, commandId,
    occurredAt: startedAt, detail: { command: ["node", ...args] }, evidenceRefs: [],
  });
  const attempts = [];
  ...
  if (isRetryableBindingChangedResult(result)) {
    ...
    emitWorkerEvent({
      event: "retry_started", status: "running", alias, actionId, commandId, retryIndex: 1,
      occurredAt: attemptStartedAt, detail: { reason: "binding_snapshot_changed" }, evidenceRefs: [],
    });
    result = runNode(args, { allowFail: true, timeoutMs });
    const retryEndedAt = new Date().toISOString();
    const retryDurationMs = Date.now() - attemptStartedMs;
    attempts.push({
      startedAt: attemptStartedAt,
    ...
    emitWorkerEvent({
      event: "retry_finished", status: result.status === 0 ? "succeeded" : "failed",
      alias, actionId, commandId, retryIndex: 1, occurredAt: retryEndedAt,
      detail: { durationMs: retryDurationMs, exitCode: result.status }, evidenceRefs: [],
    });
  }
  const record = {
    alias,
  ...
  const recordPath = join(logDir, `${ordinal}-${actionId}.json`);
  writeJson(recordPath, record);
  emitWorkerEvent({
    event: "command_finished", status: result.status === 0 ? "succeeded" : "failed",
    alias, actionId, commandId, occurredAt: record.endedAt,
    detail: { durationMs: record.durationMs, exitCode: record.exitCode, retryCount: record.retryCount },
    evidenceRefs: [relative(ROOT, recordPath).replaceAll("\\", "/")],
  });
  if (result.status !== 0) {
...
async function workerMain(args) {
  ...
  process.on("message", handleWorkerParentMessage);
  console.log(`WORKER_START alias=${alias} sequences=${sequences.length}`);
  try {
    await persistWorkerEvent({
      event: "worker_started", status: "running", alias, occurredAt: result.startedAt,
      detail: { actor, sequenceCount: sequences.length }, evidenceRefs: [],
    });
    for (let sequenceIndex = 0; sequenceIndex < sequences.length; sequenceIndex += 1) {
      assertWorkerEvidenceHealthy();
      ...
      await persistWorkerEvent({
        event: "sequence_started", status: "running", alias, sequenceIndex,
        detail: { order: sequence }, evidenceRefs: [],
      });
      for (let actionIndex = 0; actionIndex < sequence.length; actionIndex += 1) {
        ...
        await persistWorkerEvent({
          event: "action_started", status: "running", alias, sequenceIndex, actionId,
          occurredAt: actionStartedAt, detail: { actionIndex }, evidenceRefs: [],
        });
        ...
        if (actionId === "browse_feed") {
          ...
          assertZeroEffectCommand(actionId, command);
        ...
        const actionDurationMs = Date.now() - startedMs;
        sequenceResult.actions.push({ actionId, ok: true, durationMs: actionDurationMs });
        console.log(`ACTION_OK alias=${alias} sequence=${sequenceIndex} action=${actionId}`);
        await persistWorkerEvent({
          event: "action_finished", status: "succeeded", alias, sequenceIndex, actionId,
          detail: { actionIndex, durationMs: actionDurationMs }, evidenceRefs: [],
        });
        assertWorkerEvidenceHealthy();
      }
      ...
      await persistWorkerEvent({
        event: "sequence_finished", status: "succeeded", alias, sequenceIndex,
        detail: { order: sequence }, evidenceRefs: [],
      });
    }
    await persistWorkerEvent({ event: "cleanup_started", status: "running", alias, detail: { reason: "terminal_restore_home" }, evidenceRefs: [] });
    ...
    await persistWorkerEvent({ event: "cleanup_finished", status: "succeeded", alias, detail: { reason: "terminal_restore_home" }, evidenceRefs: [] });
    result.ok = true;
  } catch (error) {
    ...
    try {
      await persistWorkerEvent({
        event: result.stopClass === "evidence_failure" ? "evidence_failure" : "stop_requested",
        status: result.stopClass, alias, detail: { reason: result.reason }, evidenceRefs: [],
      });
    } catch { /* the parent already owns the evidence failure */ }
    if (result.stopClass !== "risk_or_unknown_surface") {
      await runFailureCleanup({
        persistEvent: (event) => persistWorkerEvent({
          event, status: event === "cleanup_finished" ? "succeeded" : "running", alias,
          detail: { reason: "failure_cleanup" }, evidenceRefs: [],
        }),
        restore: () => restoreXhsHome({ alias, sessionFile, logDir: workDir, label: "failure-cleanup" }),
      });
    }
  } finally {
    ...
    try {
      await persistWorkerEvent({
        event: "worker_finished", status: result.ok ? "succeeded" : result.stopClass,
        alias, occurredAt: result.endedAt, detail: { reason: result.reason, effects: result.effects },
        evidenceRefs: [relative(ROOT, resultPath).replaceAll("\\", "/")],
      });
    } catch { /* raw result remains the fallback */ }
  }
  console.log(`WORKER_END alias=${alias} ok=${result.ok}`);
  return result.ok ? 0 : 1;
}

function spawnWorker({ alias, actor, runId, attempt, parentNonce, sessionFile, sequences, keyword, onEvent, onChild }) {
  ...
  child.on("message", (message) => {
    if (message?.type !== "xhs-run-event") return;
    const event = onEvent?.(message.draft, child) || null;
    try {
      child.send({
        type: "xhs-run-event-ack",
        requestId: message.requestId,
        ok: Boolean(event),
        event,
        error: event ? null : "parent did not persist the event",
      });
    } catch { /* the worker will fail its acknowledgement timeout */ }
  });
  const prefix = `[${alias}] `;
...
async function parentMain(args) {
  ...
  const summary = {
    ...
    effectEnforcement: { mode: "exact_command_dry_run_allowlist", verifiedAtDispatch: true },
  ...
  const parentCreatedAt = JSON.parse(readFileSync(join(root, "parent.json"), "utf8")).createdAt;
  const stopPath = join(root, "STOP.json");
  ...
  const writer = createRunEventWriter({
    path: join(root, "report", "timeline.jsonl"),
    runId,
    attempt,
    source: "parent",
    mode: "live",
    onPersisted: (event) => console.log(formatLiveEvent(event)),
  });
  const failEvidence = (error) => {
    if (evidenceFailure) return;
    evidenceFailure = error instanceof Error ? error : new Error(String(error));
    signalGlobalStop(stopPath, {
      alias: null,
      stage: "timeline",
      code: "evidence_failure",
      detail: String(evidenceFailure.message || evidenceFailure).slice(0, 500),
      observedAt: new Date().toISOString(),
    });
    for (const child of children) {
      try { child.send({ type: "xhs-run-evidence-failure", error: evidenceFailure.message }); } catch { /* best effort */ }
    }
  };
  const appendEvent = (draft) => {
    if (evidenceFailure) return null;
    try {
      return writer.append(draft);
    } catch (error) {
      failEvidence(error);
      return null;
    }
  ...
  appendEvent({
    event: "attempt_started", status: "running", occurredAt: parentCreatedAt,
    detail: { attemptDirectory: canaryRootName(attempt), actor },
    evidenceRefs: [relative(ROOT, join(root, "parent.json")).replaceAll("\\", "/")],
  });
  const sessions = [];
  const releases = [];
  let workerExits = [];
  ...
  try {
    ...
    for (const alias of aliases) {
      ...
      appendEvent({
        event: "session_acquired", status: "succeeded", alias,
        detail: { capabilityId: summary.capabilityId }, evidenceRefs: [],
      });
      if (evidenceFailure) throw evidenceFailure;
    }
    for (const session of sessions) {
      ...
      appendEvent({ event: "preflight_completed", status: "succeeded", alias: session.alias, detail: {}, evidenceRefs: [] });
      if (evidenceFailure) throw evidenceFailure;
    }
    console.log(`CONCURRENCY_START aliases=${aliases.join(",")} workers=${aliases.length} transportTrueSplit=false`);
    appendEvent({
      event: "concurrency_started", status: "running",
      detail: { aliases, workers: aliases.length, transportTrueSplit: false }, evidenceRefs: [],
    });
    const startedMs = Date.now();
    const activeWorkers = new Set(aliases);
    const ticker = setInterval(() => {
      appendEvent({
        event: "heartbeat", status: "running",
        detail: { elapsedSec: Math.floor((Date.now() - startedMs) / 1000), activeWorkers: activeWorkers.size, aliases: [...activeWorkers] },
        evidenceRefs: [],
      });
    }, 30_000);
    try {
      workerExits = await Promise.all(sessions.map(async (session) => {
        const exit = await spawnWorker({
          ...
          keyword,
          onEvent: (draft) => appendEvent(draft),
          onChild: (child) => {
            children.add(child);
        ...
        const release = { alias: session.alias, ...releaseSession(session.path) };
        releases.push(release);
        appendEvent({
          event: "session_released", status: release.ok ? "succeeded" : "failed", alias: session.alias,
    ...
    parentError = error;
    if (error?.code === "XHS_RUN_EVENT_WRITE_FAILED") failEvidence(error);
    if (!evidenceFailure) {
      const stop = {
        alias: null,
        stage: "parent",
      ...
      signalGlobalStop(stopPath, stop);
      appendEvent({ event: "stop_requested", status: "parent_failure", occurredAt: stop.observedAt, detail: stop, evidenceRefs: [] });
    }
  } finally {
    for (const session of sessions) {
  ...
  const output = {
    ...summary,
    schemaId: "xhs.compose-conc4-canary-execution-summary.v1",
    schemaVersion: 1,
    planSchemaId: summary.schemaId,
    ok: !parentError && !evidenceFailure && results.every((result) => result.ok),
    mode: "execute",
    runId,
    workerExits,
    results,
    ...
    parentError: parentError ? String(parentError.message || parentError).slice(0, 500) : null,
    evidenceFailure: evidenceFailure ? String(evidenceFailure.message || evidenceFailure).slice(0, 500) : null,
  };
  appendEvent({
    event: "attempt_finished", status: output.ok ? "passed" : evidenceFailure ? "evidence_failed" : output.stop ? "controlled_stop" : "failed",
    detail: { rawOk: output.ok, stop: output.stop, parentError: output.parentError }, evidenceRefs: [],
  });
  writeJson(join(root, "summary.json"), output);
  try {
    finalizeLiveAttemptReport({ repoRoot: ROOT, runId, attemptRoot: root, evidenceFailure });
    buildRunReport({ repoRoot: ROOT, runId });
  } catch (error) {
    failEvidence(error);
    output.ok = false;
    output.evidenceFailure = String(error?.message || error).slice(0, 500);
    output.stop = existsSync(stopPath) ? JSON.parse(readFileSync(stopPath, "utf8")) : output.stop;
    writeJson(join(root, "summary.json"), output);
  }
