...
if (flag("--help") || flag("-h")) {
  console.log(`用法: node ops/xhs-dm-user.mjs --alias <01-04> --session-file <context.json> --user <昵称> [--text 你好] [--send]
默认打开私信页；--send 才发送。`);
  process.exit(0);
}
...
function fail(reason, extra = {}) {
  ...
  bizRecord({ op: "dm_user", outcome: "fail", reason, extra, alias, serial: null, startMs: t0 });
  process.exit(2);
}
...
async function main() {
  ...
  if (!doSend) {
    ...
    console.log(`ALIAS=${alias}`);
    bizRecord({ op: "dm_user", outcome: "ok", reason: "typed-no-send", alias, serial: null, startMs: t0 });
    process.exit(0);
  }
  ...
  console.log(`ALIAS=${alias}`);
  bizRecord({ op: "dm_user", outcome: "ok", reason: "sent", alias, serial: null, startMs: t0 });
  process.exit(0);
}
...
main().catch((e) => {
  ...
  console.log(`ALIAS=${alias}`);
  bizRecord({ op: "dm_user", outcome: "fail", reason: "exception", extra: { detail: String(e.message || e).slice(0, 300) }, alias, serial: null, startMs: t0 });
  process.exit(4);
});
