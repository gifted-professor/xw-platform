...
if (flag("--help") || flag("-h")) {
  console.log(`用法: node ops/douyin-like-set.mjs --session-dir <contexts-dir> [--aliases 01,02] [--no-force-stop]
多机串行 douyin-like --dry-run；汇总 PASS/FAIL。默认 aliases=01,02（勿加未登录机）。`);
  process.exit(0);
}
...
async function main() {
  ...
  for (const alias of aliases) {
    ...
    const args = ["ops/douyin-like.mjs", "--alias", alias, "--session-file", resolve(sessionDir, `${alias}.json`), "--dry-run", "--ssh", ssh];
    if (!forceStop) args.push("--no-force-stop");
    const r = await runOps(args, 180000);
    const k = kv(r.out);
    const result = k.DOUYIN_LIKE || (r.code === 0 ? "ok" : "fail");
  ...
  console.log(`DOUYIN_LIKE_SET=${setOutcome}`);
  console.log(`ALIASES=${aliases.join(",")}`);
...
main().catch((e) => {
  console.log(`DOUYIN_LIKE_SET=fail`);
