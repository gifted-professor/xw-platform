...
统一源码项目。当前仍处于 **Physical Fusion** 阶段。

## 当前状态

- XW Platform 是两旧仓（`xhs-registry`、`xhs-device-agent`）的统一源码主线。
- 当前仍处于 Physical Fusion（源码合仓）阶段，**不是**运行时统一阶段。
...
| 阶段 | 内容 | 状态 |
|---|---|---|
| F1-B | 骨架文档 + 合同/source-lock 副本 | 进行中 |
| F1-C | 导入 Orchestrator（xhs-registry 历史） | 待 |
| F1-D | 导入 Control Plane（xhs-device-agent 历史） | 待 |
| F1-E | 增加导入验证工具 | 待 |
| F1-F | 增加根命令和离线 CI | 待 |
| F1-G | 完成 Physical Fusion 验收 | 待 |

## 更多

- `docs/architecture/target-layout.md` — 目标目录结构。
- `docs/architecture/runtime-boundaries.md` — 运行时边界与不变量。
- `docs/fusion/` — 合仓合同、source lock、bootstrap receipt。